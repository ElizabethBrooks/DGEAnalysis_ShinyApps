      ...
      conditionalPanel(
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            fluidRow(
              column(
                ...
                actionButton("runSetMinSize", "Run"),
              ),
              column(
                width = 6,
                sliderInput(
                ...
                actionButton("runSetCutHeight", "Run"),
              )
            ),
            #tags$p(
              #align="center",
          ...
          tabPanel(
            ...
            actionButton("runSetPowersRange", "Run"),
            tags$br(),
            ...
            actionButton("runSetMEDissThres", "Run"),
            tags$br(),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$runSetCutHeight, {
  ...
  observeEvent(input$runSetMinSize, {
    # retrieve cluster of samples
    sampleTree <- createSampleTree()
    # setup test maximum cut height
    numSamples <- length(sampleTree$height)+1
  ...
  # update power inputs
  observeEvent(input$runSetPowers, {
